...
void M_ScanSaves (void)
{
	...
	for (i=0 ; i<MAX_SAVEGAMES ; i++)
	{
		...
		fread (&fscanf,1, 1, f);
		//sprintf(version,"%i\n",fscanf);
		version = 5; //it better had be!		
		//fread (&version,sizeof(int), 1, f);
		
		int newline = fgetc(f);
		
		if(newline != '\n')
			Sys_Error("not a newline");
...
void M_Options_Draw (void)
{
	...
	M_Print (16, 128, "            Dingoo CPU    ");
	
	char cpu_chr[4]; sprintf(cpu_chr,"%i",DINGOO_OVERCLOCK);
	M_Print (219, 128, cpu_chr);
...
// cursor
	if(options_cursor==13)
		M_DrawCharacter (200, 32 + (options_cursor-1)*8, 12+((int)(realtime*4)&1));
	else
...
void M_Options_Key (int k)
{
	...
	case K_CTRL:	
		...
		case 13:
			//M_Menu_Video_f ();
			sprintf(ctxt, "%s%i%s", "Press START to set\nthe cpu to ", 
									DINGOO_OVERCLOCK, 
										"Mhz\nPress SELECT to cancel."); 
			if (!SCR_ModalMessage(ctxt))
				break;
			//cpu_clock_set((DINGOO_OVERCLOCK*1000000));
			break;
		default:
			M_AdjustSliders (1);
			break;
...
void M_Keys_Draw (void)
{
	...
	if (bind_grab)
		...
		M_Print (18, 32, "A to change, right shoulder to clear");
