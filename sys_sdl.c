...
char *basedir = "/home/iceman/tmp/quake_dingoo/";
char *cachedir = "";
...
void Sys_Error (char *error, ...)
{
    ...
    FILE* ERRFILE;
    ERRFILE = fopen("\\GAME\\QUAKE\\error.wub","w");

    va_start (argptr,error);
    vsprintf (string,error,argptr);
    va_end (argptr);
   
    fprintf(ERRFILE,"%s",string);
    fclose(ERRFILE);   
...
void Sys_Warn (char *warning, ...)
{
    ...
    FILE* WARNFILE;
    WARNFILE = fopen("\\GAME\\QUAKE\\warn.wub","w");
	
    va_start (argptr,warning);
    vsprintf (string,warning,argptr);
    va_end (argptr);
	
    fprintf(WARNFILE, "Warning: %s", string);
    fclose(WARNFILE);   
} 
...
int Sys_FileOpenWrite (char *path)
{
	...
	if (!f) {
		sprintf(dingerr,"%s%s","Error opening: ",path);
		Sys_Error (dingerr);
	}
...
{
	char *QUAKEOPT = SDL_getenv("QUAKEOPT");
...
int main (int c, char **v)
{
	...
	/*Fix the "holding down X button issue"*/	
