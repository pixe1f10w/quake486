...
//Dingoo save buffers
char BB[524288]; //Main buffer, hopefully slightly larger than needed.
char B[128]; //Temporary buffer
...
void Host_Savegame_f (void)
{
	...
	sprintf(BB, "%s", "\0"); // Reset the Dingoo save buffer
...
//Dingoo version: modified to remove calls to fscanf.
...
//	SCR_BeginLoadingPlaque ();
	...
	int chk = fgetc(f);
	if(chk != '\n')
		Sys_Error("no newline after version");
	...
	//fscanf (f, "%s\n", str);
	char fscanf[64]; //MAX_STYLESTRING and MAX_QPATH = 64
	
	i  = 0;
	do {
		fread (&fscanf[i],1, 1, f);
		if(fscanf[i] == '\n')
		{
			fscanf[i]  = '\0';
			i  = 64;
